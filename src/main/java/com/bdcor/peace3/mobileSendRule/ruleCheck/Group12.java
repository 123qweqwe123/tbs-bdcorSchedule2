...
public class Group12 extends Group {
	...
	public Group12(List<Patient> patients,CommDao commDao) {
		...
		if(dayOfMonth!=5 && dayOfMonth!=20){
			if(msgSendedByDate!=null && msgSendedByDate.sendMsg.size()>0){
				...
				for(SendMsg msg:msgSendedByDate.sendMsg){
					if(Group.notDateLimit.indexOf(msg.msgType.replace("12_", ""))==-1){
						msgCount++;
					}
				}
				if(msgCount>0){
